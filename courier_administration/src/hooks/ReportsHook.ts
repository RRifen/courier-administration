...
export function useReports() {
    ...
    async function deleteReportHandler(obj: any) {
        try {
            await axios.post(
                node_mysql,
                {query: `DELETE FROM reports WHERE report_id='${obj.report_id}'`}
            )
    ...
    async function updateReportHandler(obj: any) {
        ...
        try {
            await axios.post(
                node_mysql,
                {query: `UPDATE reports SET admin_id='${obj.admin_id}', report_type_id='${obj.report_type_id}', report_date='${obj.report_date}', report_text='${obj.report_text}' WHERE report_id='${obj.report_id}'`}
            ).catch((error) => {
    ...
    async function insertReportHandler(obj: any) {
        try {
            let response = await axios.post(
                node_mysql,
                {query: `INSERT INTO reports(admin_id, report_type_id, report_text) VALUES('${obj.admin_id}', '${obj.report_type_id}', '${obj.report_text}')`}
            ).catch((error) => {
                alert(error.response.data.error);
                throw new Error();
            })
    ...
    async function searchHandler(obj: any) {
        ...
        if (expArray.length === 0) {
            ...
            await fetchReports( "SELECT report_id, admin_id, report_type_id, description, report_date, report_text FROM reports JOIN report_types USING(report_type_id) WHERE " + expArray.join(" AND "));
        }
    }

    async function readHandler() {
        return await fetchReports("SELECT report_id, admin_id, report_type_id, description, report_date, report_text FROM reports JOIN report_types USING(report_type_id)");
    }
